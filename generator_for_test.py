...
if __name__ == "__main__":
    ...
    for target in tqdm(range(1319)):
        ...
        with open(f"prolog_test_files/{target}.pl", 'w') as prolog_file:
            prolog_file.write(prolog_code)

        try:
            complete_filename = f"prolog_test_files/{target}.pl"
            ...
            if int(consult_ans) != int(target_ans):
                os.rename(f"prolog_test_files/{target}.pl", f"prolog_test_files/{target}{SEMANTICERROR_FLAG}.pl")
        except:
            os.rename(f"prolog_test_files/{target}.pl", f"prolog_test_files/{target}{SYNTAXERROR_FLAG}.pl")