...
train_path = 'prolog_files'
test_path = 'prolog_test_files'
...
def generate_messages(questions, target, wrong_code=None):
    ...
    for idx in questions:
        question = train_examples[idx]['question'].strip()
        answer = train_examples[idx]['answer'].strip()
        with open(f'{train_path}/{idx}.pl', 'r') as prolog_file:
            prolog_code = prolog_file.read().strip()
        if wrong_code is not None:
            if os.path.exists(f'{train_path}/{idx}#.pl'):
                with open(f'{train_path}/{idx}#.pl', 'r') as prolog_file:
    ...
    target_question = test_examples[target]['question'].strip()
    target_answer = test_examples[target]['answer'].strip()
        ...
        for (root, dirs, files) in os.walk(test_path):
            for filename in tqdm(files, desc=f"Epoch {epoch}"):
                if "#" in filename:
                    ...
                    with open(f"{test_path}/{target}.pl", 'w') as prolog_file:
                        try:
                    ...
                    try:
                        assert prolog_code != ''
                        complete_filename = f"{test_path}/{target}.pl"
                        ...
                        if int(consult_ans) != int(target_ans):
                            ...
                            os.rename(f"{test_path}/{target}.pl", f"{test_path}/{target}{SEMANTICERROR_FLAG}.pl")
                        else:
                            fixed_num += 1
                            num += 1
                            os.remove(os.path.join(root, filename))
                        ...
                        os.remove(os.path.join(root, filename))
                        os.rename(f"{test_path}/{target}.pl", f"{test_path}/{target}{SYNTAXERROR_FLAG}.pl")
